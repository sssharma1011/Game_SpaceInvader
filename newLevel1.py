...
#Level 1
l1 =pygame.font.Font("freesansbold.ttf",50)
yes=pygame.font.Font("freesansbold.ttf",50)
...
class Level1:
    ...
    def level2(self,width=100, heigh=60):

        Level1=l1.render("Congratulations!!! you've reached at level 2", True,(255,255,255))
        screen.blit(Level1,(200, 250))
        yay=yes.render("YES", True,(255,255,255))
        screen.blit(yay,(400,300))
...
while running:
    ...
    if event.type==pygame.KEYDOWN:
        if event.key == pygame.K_UP:
            if bullet_state is "ready":
                ...
                bullet_x=player_x
                new_l1.fire_bullet(bullet_x,bullet_y)
                print("UP")
    ...
    if player_x<=0:
        ...
        player_x=(800-64)
    new_l1.player(player_x, player_y)

    #enemy movement

    for i in range(new_l1.num_of_enemies):
        #game over
        if enemy_y[i]>440:
            for j in range(new_l1.num_of_enemies):
                enemy_y[i]=2000
            new_l1.game_over_text()
            break
        ...
        if enemy_x[i]<=0:
            ...
            enemy_y[i]+=enemy_y_change[i]
        #collision
        collision=new_l1.isCollision(enemy_x[i],enemy_y[i],bullet_x,bullet_y)
        if collision:
        ...
        new_l1.enemy(enemy_x[i], enemy_y[i], i)
    ...
    if bullet_state is "fire":
        new_l1.fire_bullet(bullet_x, bullet_y)# bullet is not gonna appear if we remove it
        bullet_y -=bullet_y_change

    if score_val==20:
        new_l1.level2()
    new_l1.player(player_x, player_y)
    new_l1.show_score(text_x,text_y)
